...
def test_execute_upload_fans_out_opted_in_cross_core_compat(tmp_path: Path) -> None:
    """With sync.cross_core_compat = ["mednafen_psx_hw"], uploading a DuckStation
    save (per CROSS_CORE_COMPAT, reverse of the mednafen_psx_hw -> duckstation
    download mapping) also uploads a companion copy tagged emulator
    "mednafen_psx_hw" with the ".srm" extension instead of ".mcd" — the format
    other RomM clients using that core (e.g. Argosy Launcher) expect.
    """
    config = make_config(tmp_path)
    config.sync.cross_core_compat = ["mednafen_psx_hw"]
    saves_root = Path(config.emudeck.saves_path).expanduser()
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/api/saves" and request.method == "POST":
            save_id = 55 + len(uploaded)
            uploaded.append((request.content, dict(request.url.params)))
            return httpx.Response(
                200,
                json={
                    "id": save_id,
    ...
    assert b'filename="Klonoa - Door to Phantomile (USA).srm"' in fanout_body
    assert b".mcd" not in fanout_body
    assert fanout_params.get("emulator") == "mednafen_psx_hw"

    # local file on disk untouched by the fan-out
    assert local_save.exists()
    assert local_save.read_bytes() == b"local-save-data"


def test_execute_upload_skips_fanout_without_opt_in(tmp_path: Path) -> None:
    """Without sync.cross_core_compat opting "mednafen_psx_hw" in, a DuckStation
    upload is not fanned out — the reverse mapping is opt-in, same as download.
    """
    config = make_config(tmp_path)
    saves_root = Path(config.emudeck.saves_path).expanduser()
    (saves_root / "duckstation/saves").mkdir(parents=True, exist_ok=True)
